...
The CRUD endpoints are already created, for Invoices (Facturas), Credit Notes, Debit Notes, and Quotes (Presupuestos) with complete business rules for pricing, discounts, and VAT calculations, for those implemtations in the views, it is not necesary to load all the invoices, credit notes, quotes etc. When the user wants to make a new invoice, quote, credit note or debit note, the system should only load the necessary data to create that document, such as customer information, product information and pricing details. The system should not load all existing invoices, credit notes, quotes or debit notes into memory when creating a new document. This approach ensures better performance and scalability, especially as the number of documents grows over time.
