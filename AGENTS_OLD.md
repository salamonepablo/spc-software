...
|--------|---------|--------|-------|
| **Line Coverage (Core)** | 80% | 90% | Services: Pricing, Invoices, Quotes |
| **Line Coverage (Global)** | 60% | 80% | Excludes migrations, generated code |
| **Branch Coverage** | 50% | 70% | Focus on business logic branches |
| **Tests Passing** | 100% | 100% | No failing tests in develop/main |

**Coverage Scope:**
- **Core (required):** `SPC.API.Services.PricingService`, `InvoicesService`, `QuotesService`
- **Excluded:** Migrations, `*.Designer.cs`, `*.generated.cs`, Program.cs

**Quality Gates:**
1. All tests must pass before commit
2. New features require tests (TDD preferred)
3. Core coverage must stay above 80%
4. No `#pragma warning disable` without justification
...
# Generate HTML report (core only)
...
- [x] Create entity models (28 total)
- [x] Create enums and DTOs
- [x] Configure DbContext with relationships
- [x] Add test suite (111 tests)
- [x] Data migration script (Access -> SQL Server)

### Phase 2 (UI & Queries) - Completed
- [x] GET endpoints (read-only)
