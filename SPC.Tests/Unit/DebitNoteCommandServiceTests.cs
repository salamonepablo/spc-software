...
/// <summary>
/// Unit tests for DebitNoteCommandService (write debit note operations).
/// Uses InMemory database with real calculation services.
/// TDD: RED phase — these tests define expected behavior.
/// </summary>
public class DebitNoteCommandServiceTests : IDisposable
{
    private readonly SPCDbContext _db;
    private readonly IDebitNoteCommandService _commandService;
    ...
    public DebitNoteCommandServiceTests()
    {
        var options = new DbContextOptionsBuilder<SPCDbContext>()
            .UseInMemoryDatabase($"DebitNoteCommandTest_{Guid.NewGuid()}")
            .Options;
        ...
        _queryService = new DebitNoteQueryService(_db);
        ...
        _commandService = new DebitNoteCommandService(
            _db,
            taxService,
            pricingService,
            _queryService,
    ...
    [Fact]
    public async Task CreateAsync_CreatesDebitNoteA_WithCorrectTaxCalculations()
    {
        // Arrange
    ...
    public async Task CreateAsync_GeneratesSequentialNumber()
    {
        ...
        // Assert
        second.DebitNoteNumber.Should().Be(first.DebitNoteNumber + 1);
    }
    ...
    [Fact]
    public async Task VoidAsync_MarksDebitNoteAsVoided()
    {
        // Arrange
        await SeedBaseDataAsync();
        ...
        // Act
        var result = await _commandService.VoidAsync(debitNote.Id, "Error de facturación");

        // Assert
        result.Should().BeTrue();
        var voided = await _db.DebitNotes.FindAsync(debitNote.Id);
    ...
    public async Task VoidAsync_ReturnsFalse_WhenAlreadyVoided()
    {
        ...
        // Act
        var result = await _commandService.VoidAsync(debitNote.Id, "Double void");
